# My Learning Progress Tracker

> **How to use:** Fork this repo, then edit this file in your fork to check off items as you complete them. Commit your updates regularly — your progress history becomes part of your learning portfolio!

---

## Getting Started

- [ ] Read [Role Comparison](00_Overview/role_comparison.md) and chose my track
- [ ] Read [How to Use This Repo](00_Overview/how_to_use.md)
- [ ] Set up my local environment (`pip install -r requirements.txt`)
- [ ] Launched JupyterLab successfully

---

## Foundations (Recommended for All Tracks)

### Mathematics
- [ ] `linear_algebra.ipynb` — Vectors, matrices, eigenvalues
- [ ] `calculus_for_ml.ipynb` — Derivatives, gradients, chain rule
- [ ] `probability_theory.ipynb` — Bayes' theorem, distributions
- [ ] `information_theory.ipynb` — Entropy, KL divergence
- [ ] `optimization.ipynb` — Gradient descent variants
- [ ] `dimensionality_reduction.ipynb` — PCA, SVD
- [ ] `graph_theory.ipynb` — Graphs, shortest paths

### Statistics
- [ ] `descriptive_statistics.ipynb`
- [ ] `probability_distributions.ipynb`
- [ ] `hypothesis_testing.ipynb`
- [ ] `regression_analysis.ipynb`
- [ ] `experimental_design.ipynb`

### Programming
- [ ] Read [Python for DS guide](04_Foundations/programming/python_for_ds.md)
- [ ] Read [SQL Basics guide](04_Foundations/programming/sql_basics.md)

---

## Track 1: Data Scientist

### Beginner
- [ ] `01_python_for_ds.ipynb`
- [ ] `02_pandas_and_numpy.ipynb`
- [ ] `03_data_cleaning.ipynb`
- [ ] `04_exploratory_data_analysis.ipynb`
- [ ] `05_data_visualization.ipynb`
- [ ] `06_sql_for_data_science.ipynb`
- [ ] Completed at least 1 beginner mini-project

### Intermediate
- [ ] `07_statistics_for_ml.ipynb`
- [ ] `08_feature_engineering.ipynb`
- [ ] `09_supervised_learning.ipynb`
- [ ] `10_model_evaluation.ipynb`
- [ ] `11_unsupervised_learning.ipynb`
- [ ] `12_advanced_visualization.ipynb`
- [ ] Completed at least 1 intermediate mini-project

### Advanced
- [ ] `13_deep_learning_intro.ipynb`
- [ ] `14_natural_language_processing.ipynb`
- [ ] `15_computer_vision.ipynb`
- [ ] `16_time_series.ipynb`
- [ ] `17_ab_testing.ipynb`
- [ ] `18_bayesian_methods.ipynb`
- [ ] `19_causal_inference.ipynb`
- [ ] `20_ml_interpretability.ipynb`
- [ ] Built and published a capstone project

---

## Track 2: ML Engineer

### Beginner
- [ ] `01_python_and_git_for_mle.ipynb`
- [ ] `02_docker_fundamentals.ipynb`
- [ ] `03_ml_pipelines_sklearn.ipynb`
- [ ] `04_api_development_fastapi.ipynb`
- [ ] `05_cloud_basics.ipynb`
- [ ] `06_testing_ml_code.ipynb`
- [ ] Completed at least 1 beginner mini-project

### Intermediate
- [ ] `01_mlflow_experiment_tracking.ipynb`
- [ ] `02_feature_stores.ipynb`
- [ ] `03_model_serving.ipynb`
- [ ] `04_monitoring_and_drift.ipynb`
- [ ] Completed at least 1 intermediate mini-project

### Advanced
- [ ] `01_distributed_training.ipynb`
- [ ] `02_kubernetes_for_ml.ipynb`
- [ ] `03_llm_finetuning_pipeline.ipynb`
- [ ] `04_ml_platform_design.ipynb`
- [ ] Built and published a production ML pipeline project

---

## Track 3: AI Engineer

### Beginner
- [ ] `01_llm_apis.ipynb`
- [ ] `02_prompt_engineering.ipynb`
- [ ] `03_chatbot_development.ipynb`
- [ ] `04_basic_rag.ipynb`
- [ ] `05_ai_app_deployment.ipynb`
- [ ] Completed at least 1 beginner mini-project

### Intermediate
- [ ] `01_advanced_rag.ipynb`
- [ ] `02_langchain_deep_dive.ipynb`
- [ ] `03_vector_databases.ipynb`
- [ ] `04_ai_agents.ipynb`
- [ ] `05_llamaindex.ipynb`
- [ ] `06_llm_evaluation.ipynb`
- [ ] `07_production_ai_systems.ipynb`
- [ ] Completed at least 1 intermediate mini-project

### Advanced
- [ ] `01_finetuning_alignment.ipynb`
- [ ] `02_multi_agent_systems.ipynb`
- [ ] `03_safety_and_guardrails.ipynb`
- [ ] `04_advanced_retrieval.ipynb`
- [ ] Built and published an AI-powered application

---

## Hands-On Labs

### Completed Notebooks
- [ ] `01_pandas_fundamentals.ipynb`
- [ ] `02_stats_intuition.ipynb`
- [ ] `03_data_visualization.ipynb`
- [ ] `04_sql_for_analysts.ipynb`
- [ ] `05_llm_api_basics.ipynb`
- [ ] `06_rag_from_scratch.ipynb`
- [ ] `07_feature_engineering.ipynb`
- [ ] `08_mlops_fastapi_deployment.ipynb`
- [ ] `09_neural_network_scratch.ipynb`
- [ ] `10_pytorch_intro.ipynb`
- [ ] `13_prompt_engineering.ipynb`
- [ ] `14_fine_tune_bert.ipynb`
- [ ] `15_llm_fine_tuning_lora.ipynb`
- [ ] `16_mlops_pipeline.ipynb`
- [ ] `17_multi_agent_system.ipynb`
- [ ] `19_transformer_from_scratch.ipynb`
- [ ] `20_llm_eval_framework.ipynb`

### Community Challenges
- [ ] Challenge 01: Titanic Survival Prediction
- [ ] Challenge 02: Build a RAG Pipeline from Scratch
- [ ] Challenge 03: Deploy an ML Model with FastAPI + Docker

---

## Interview Prep

### Data Scientist
- [ ] Completed all 20+ Q&A in `06_Interview_Prep/data_scientist/`
- [ ] Completed SQL challenges section
- [ ] Completed coding challenges section
- [ ] Done at least 1 mock interview

### ML Engineer
- [ ] `coding_challenges.ipynb`
- [ ] `ml_concepts_qa.ipynb`

### AI Engineer
- [ ] `llm_concepts.ipynb`
- [ ] `rag_from_scratch.ipynb`

---

## Hackathons (10_Hackathons)

### Theme 1 — Prolonged Coordination
- [ ] Read [Theme 1 README](10_Hackathons/theme_1_prolonged_coordination/README.md)
- [ ] Reviewed all 33 ideas in [ideas.md](10_Hackathons/theme_1_prolonged_coordination/ideas.md)
- [ ] Built the ReasoningBank skeleton: [starter_code/01_reasoningbank_skeleton.py](10_Hackathons/deep_dive_ideas/starter_code/01_reasoningbank_skeleton.py)

### Theme 2 — Multi-Agent Collaboration
- [ ] Read [Theme 2 README](10_Hackathons/theme_2_multi_agent_collaboration/README.md)
- [ ] Reviewed all 44 ideas in [ideas.md](10_Hackathons/theme_2_multi_agent_collaboration/ideas.md)
- [ ] Built the A2A handshake skeleton: [starter_code/04_a2a_handshake.py](10_Hackathons/deep_dive_ideas/starter_code/04_a2a_handshake.py)

### Theme 3 — Adaptive Retrieval
- [ ] Read [Theme 3 README](10_Hackathons/theme_3_adaptive_retrieval/README.md)
- [ ] Reviewed all 23 ideas in [ideas.md](10_Hackathons/theme_3_adaptive_retrieval/ideas.md)
- [ ] Built the ColPali indexing skeleton: [starter_code/07_colpali_index.py](10_Hackathons/deep_dive_ideas/starter_code/07_colpali_index.py)
- [ ] Built the GraphRAG + HippoRAG skeleton: [starter_code/09_graphrag_query.py](10_Hackathons/deep_dive_ideas/starter_code/09_graphrag_query.py)

### Deep Dive Blueprints
- [ ] Read blueprint README: [deep_dive_ideas/README.md](10_Hackathons/deep_dive_ideas/README.md)
- [ ] Blueprint 01: [Viral Autopsy](10_Hackathons/deep_dive_ideas/01_viral_autopsy.md)
- [ ] Blueprint 02: [Replicant](10_Hackathons/deep_dive_ideas/02_replicant.md)
- [ ] Blueprint 03: [Portfall](10_Hackathons/deep_dive_ideas/03_portfall.md)
- [ ] Blueprint 04: [Tipping Oracle](10_Hackathons/deep_dive_ideas/04_tipping_oracle.md)
- [ ] Blueprint 05: [TruthWeight](10_Hackathons/deep_dive_ideas/05_truthweight.md)
- [ ] Blueprint 06: [ChronoLaw](10_Hackathons/deep_dive_ideas/06_chronolaw.md)
- [ ] Blueprint 07: [Ghostwriter Forensics](10_Hackathons/deep_dive_ideas/07_ghostwriter_forensics.md)
- [ ] Blueprint 08: [Exodus Mapper](10_Hackathons/deep_dive_ideas/08_exodus_mapper.md)
- [ ] Blueprint 09: [Protocol Darwin](10_Hackathons/deep_dive_ideas/09_protocol_darwin.md)
- [ ] Blueprint 10: [Carbon Lie Detector](10_Hackathons/deep_dive_ideas/10_carbon_lie_detector.md)

---

## Recent Topics (11_Recent_Topics)

### Trend Tracker & Stack Watch
- [ ] Read [2025–2026 Trends](11_Recent_Topics/2025_2026_trends.md)
- [ ] Read [Stack Watch](11_Recent_Topics/stack_watch.md)

### Paper Digests (12 papers)
- [ ] [ReasoningBank](11_Recent_Topics/paper_digests/reasoningbank.md)
- [ ] [MIRIX](11_Recent_Topics/paper_digests/mirix.md)
- [ ] [Zep Temporal KG](11_Recent_Topics/paper_digests/zep.md)
- [ ] [SagaLLM](11_Recent_Topics/paper_digests/sagallm.md)
- [ ] [VIGIL](11_Recent_Topics/paper_digests/vigil.md)
- [ ] [Magentic-One](11_Recent_Topics/paper_digests/magentic_one.md)
- [ ] [A2A Protocol](11_Recent_Topics/paper_digests/a2a_protocol.md)
- [ ] [MCP](11_Recent_Topics/paper_digests/mcp.md)
- [ ] [ColPali](11_Recent_Topics/paper_digests/colpali.md)
- [ ] [Search-R1](11_Recent_Topics/paper_digests/search_r1.md)
- [ ] [GraphRAG](11_Recent_Topics/paper_digests/graphrag.md)
- [ ] [HippoRAG 2](11_Recent_Topics/paper_digests/hipporag.md)

---

## Career & Portfolio

- [ ] Read [Portfolio Guide](08_Career_Guide/portfolio_guide.md)
- [ ] Created a portfolio GitHub README
- [ ] Published at least 3 projects on GitHub
- [ ] Updated LinkedIn with new skills
- [ ] Read [Salary Guide](00_Overview/salary_guide.md) for my target role
- [ ] Joined at least 1 community (Discord, Slack, etc.)

---

## My Notes

> Use this section for personal reflections, resources you found helpful, or anything you want to remember.

### Key Insights

*(Add your own notes here)*

### Useful Links I Found

*(Add links here)*

### Projects I Want to Build

*(Add project ideas here)*

---

*Template from [AI/ML/DS Learning Hub](https://github.com/viraj97-sl/ai-ml-ds-learning-hub) · Fork and make it yours!*
