<div align="center">

# The Complete AI/ML/Data Science Learning Hub

### Your definitive roadmap from zero to career-ready — for Data Scientists, ML Engineers & AI Engineers

[![Stars](https://img.shields.io/github/stars/yourusername/ai-ml-ds-hub?style=for-the-badge&logo=github&color=FFD700)](https://github.com/yourusername/ai-ml-ds-hub)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg?style=for-the-badge)](LICENSE)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=for-the-badge)](CONTRIBUTING.md)
[![Last Updated](https://img.shields.io/badge/Last%20Updated-2025-blue.svg?style=for-the-badge)](.)
[![Beginner Friendly](https://img.shields.io/badge/Beginner-Friendly-orange.svg?style=for-the-badge)](00_Overview/how_to_use.md)

<br/>

> **"The best time to start was yesterday. The second best time is now."**

<br/>

**3 Career Tracks** | **100+ Curated Resources** | **50+ Hands-On Projects** | **Interview Prep** | **Salary Guide**

</div>

---

## Why This Repo?

The internet is full of scattered tutorials. This repo is different:

- **Structured pathways** — not random links, but a curated journey from beginner to expert
- **Role-specific** — tailored tracks for 3 distinct, high-demand careers
- **Hands-on first** — learn by building real projects, not just watching videos
- **Industry-aligned** — skills and stacks that companies actually hire for (2024-2025)
- **Free** — 95% of resources linked are free or open-source

---

## Who Is This For?

| You Are | You Should Go To |
|---------|-----------------|
| Curious beginner, no CS background | [Start Here →](00_Overview/how_to_use.md) |
| CS student exploring career options | [Role Comparison →](00_Overview/role_comparison.md) |
| Aspiring Data Scientist | [DS Track →](01_Data_Scientist/) |
| Aspiring ML Engineer | [MLE Track →](02_ML_Engineer/) |
| Aspiring AI Engineer | [AIE Track →](03_AI_Engineer/) |
| Preparing for interviews | [Interview Prep →](06_Interview_Prep/) |
| Looking for projects to build | [Hands-On Labs →](05_Hands_On_Labs/) |
| Negotiating a salary | [Salary Guide →](08_Career_Guide/salary_guide.md) |

---

## The Three Pillars

```
┌─────────────────────────────────────────────────────────────────────┐
│                                                                     │
│   DATA SCIENTIST          ML ENGINEER           AI ENGINEER         │
│   ─────────────           ───────────           ───────────         │
│                                                                     │
│   "What does            "How do we              "How do we          │
│    the data say?"        build & ship           build products      │
│                          ML systems?"           with AI?"           │
│                                                                     │
│   Statistics +           Software Eng +         LLMs + APIs +       │
│   ML + Insights          MLOps + Scale          Agents + UX         │
│                                                                     │
│   [Explore →]            [Explore →]            [Explore →]         │
└─────────────────────────────────────────────────────────────────────┘
```

> Not sure which to choose? Read the [in-depth comparison →](00_Overview/role_comparison.md)

---

## Learning Tracks

### Track 1: Data Scientist
**"Turn data into decisions"**

| Phase | Topics | Duration (self-paced) |
|-------|--------|----------------------|
| [Beginner](01_Data_Scientist/beginner/) | Python, SQL, Statistics, EDA | 2-3 months |
| [Intermediate](01_Data_Scientist/intermediate/) | ML Algorithms, Feature Engineering, Visualization | 3-4 months |
| [Advanced](01_Data_Scientist/advanced/) | Deep Learning, NLP, Time Series, A/B Testing | 4-6 months |
| [Projects](01_Data_Scientist/projects/) | End-to-end capstone projects | Ongoing |

**[→ Start Data Scientist Track](01_Data_Scientist/README.md)**

---

### Track 2: ML Engineer
**"Build systems that learn at scale"**

| Phase | Topics | Duration (self-paced) |
|-------|--------|----------------------|
| [Beginner](02_ML_Engineer/beginner/) | Python, Git, Docker, ML Basics | 2-3 months |
| [Intermediate](02_ML_Engineer/intermediate/) | MLflow, Feature Stores, Model APIs | 3-4 months |
| [Advanced](02_ML_Engineer/advanced/) | Kubernetes, Spark, LLM Fine-tuning, Ray | 4-6 months |
| [Projects](02_ML_Engineer/projects/) | Production ML pipelines | Ongoing |

**[→ Start ML Engineer Track](02_ML_Engineer/README.md)**

---

### Track 3: AI Engineer
**"Build intelligent products with AI"**

| Phase | Topics | Duration (self-paced) |
|-------|--------|----------------------|
| [Beginner](03_AI_Engineer/beginner/) | Python, APIs, Prompt Engineering | 1-2 months |
| [Intermediate](03_AI_Engineer/intermediate/) | RAG, LangChain, Vector DBs, Agents | 2-3 months |
| [Advanced](03_AI_Engineer/advanced/) | Fine-tuning, Multi-agent systems, Evals | 3-5 months |
| [Projects](03_AI_Engineer/projects/) | AI-powered apps & agents | Ongoing |

**[→ Start AI Engineer Track](03_AI_Engineer/README.md)**

---

## Shared Foundations

Before diving into any track, build these foundations:

- **[Mathematics & Statistics](04_Foundations/mathematics/)** — Linear algebra, calculus, probability
- **[Python Programming](04_Foundations/programming/)** — From syntax to software engineering
- **[Statistics for DS/ML](04_Foundations/statistics/)** — Hypothesis testing, distributions, Bayesian thinking

---

## Hands-On Labs

> Learning by doing is 10x more effective than passive consumption.

- **[Interactive Notebooks](05_Hands_On_Labs/notebooks/)** — Jupyter notebooks with exercises and solutions
- **[Mini-Projects](05_Hands_On_Labs/mini_projects/)** — 1-3 day projects to cement concepts
- **[Capstone Projects](01_Data_Scientist/projects/)** — Portfolio-worthy end-to-end projects

---

## Career Resources

- **[Salary Guide 2025](08_Career_Guide/salary_guide.md)** — Compensation by role, level, location
- **[Interview Prep](06_Interview_Prep/)** — 200+ questions with answers by role
- **[Portfolio Guide](08_Career_Guide/portfolio_guide.md)** — How to stand out to hiring managers
- **[Resume Templates](08_Career_Guide/resume_guide.md)** — ATS-friendly templates per role
- **[Networking Guide](08_Career_Guide/networking_guide.md)** — How to land the job without applying blindly

---

## Curated Resources

- **[Books](07_Resources/books.md)** — The definitive reading list (free PDFs where available)
- **[Courses](07_Resources/courses.md)** — Best free and paid courses ranked
- **[Research Papers](07_Resources/papers.md)** — Landmark papers every practitioner should read
- **[Datasets](07_Resources/datasets.md)** — 50+ datasets for practice projects
- **[Tools & Stack](07_Resources/tools_and_stack.md)** — The modern AI/ML/DS toolbox
- **[Communities](07_Resources/communities.md)** — Discord, Slack, forums, and Meetups

---

## Progress Tracker

Use the skill badge system to track your journey:

| Badge | Level | Criteria |
|-------|-------|----------|
| Novice | Just starting | Completed beginner section + 1 project |
| Practitioner | Building skills | Completed intermediate + 3 projects |
| Professional | Job-ready | Completed advanced + portfolio + interview ready |
| Expert | Senior-level | Contributing to open source, mentoring others |

> **Tip:** Fork this repo and check off completed items in each section's README!

---

## How to Use This Repo

1. **Read** the [Role Comparison](00_Overview/role_comparison.md) to pick your track
2. **Follow** the roadmap for your chosen track (beginner → intermediate → advanced)
3. **Build** at least one project per phase — add them to your GitHub
4. **Practice** with interview prep questions as you go
5. **Contribute** your learnings back (see [Contributing](CONTRIBUTING.md))

---

## Contributing

This is a living document. Contributions are warmly welcomed!

- Found a broken link? Open an issue.
- Have a better resource? Submit a PR.
- Want to add a project? See [contributing guide](CONTRIBUTING.md).

---

## License

MIT License — free to use, share, and build upon with attribution.

---

<div align="center">

**If this repo helped you, please give it a ⭐ — it helps others find it!**

---

## 💖 Contributors

A massive thank you to everyone who has contributed to this learning hub! 

<a href="https://github.com/viraj97-sl/ai-ml-ds-learning-hub/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=viraj97-sl/ai-ml-ds-learning-hub&v=1" alt="Contributors list"/>
</a>

Made with passion for the learning community | Updated March 2026

</div>
